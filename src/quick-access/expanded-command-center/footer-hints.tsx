 ...
 */
export function footerHintsFor(tab: Tab, nested: boolean, mode: CommandCenterEditMode = "normal"): readonly FooterHint[] {
  if (mode === "move") return [
...
export function CommandCenterFooterHints({ tab, nested = false, mode = "normal" }: {
  tab: Tab;
  nested?: boolean;
  mode?: CommandCenterEditMode;
}) {
  return <>
    {footerHintsFor(tab, nested, mode).map((hint, index) =>
