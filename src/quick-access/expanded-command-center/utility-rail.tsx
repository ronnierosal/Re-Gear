...
.rg-utility-slider{display:flex;flex:1 1 0;min-height:0;flex-direction:column;align-items:center;justify-content:center;gap:2px;background:transparent;border:0;border-radius:0;box-shadow:none;padding:3px 0;overflow:visible}
...
export function UtilityRail({side,layout=defaultUtilityLayout,readings={},onRequest,Button="button",Focusable="aside"}:UtilityRailProps) {
  ...
  return <Focusable flow-children="vertical" noFocusRing className="rg-utility-rail" data-utility-side={side} aria-label={`${side} quick controls`} onKeyDown={navigate}>
    ...
    {layout.filter(i=>i.side===side).map(({id})=>{
      ...
      return isSlider ? <label key={id} data-utility-id={id} data-ec-control={`utility-${id}`} className="rg-utility-control rg-utility-slider" title={reason} aria-busy={waiting||undefined}>
      ...
      </label> : <Button key={id} type="button" data-utility-id={id} data-ec-control={`utility-${id}`} className="rg-utility-control" disabled={disabled||waiting} title={reason} aria-label={`${labels[id]}: ${status}`} aria-busy={waiting||undefined} onClick={()=>request(id)}>
        <span className="rg-utility-icon"><UtilityIcon id={id}/></span>
        <span className="rg-utility-label">{labels[id]}</span>
        <span className="rg-utility-value" role="status">{status}</span>
      </Button>;
