...
## 2026-09-14 four-task follow-up (source only)

- Keep immutable 0.3.107 ZIP at source `6f64acd046730e98749fd61d544d859d686d2abd`; SHA256 `7c4df03ec3a31583c81805a4f745be1174d9b5df54d5427449e62a48e051c3fb`. It remains local, not transferred or installed. These later changes do not alter or reuse that artifact/version reservation.
- Integrated independently accepted software reconnect UI removal `bf5c5b24` as `a369f1e`, and the narrow backend exclusion `d28a34ed` as `eeb0ede`. Historical observation is retained; no reconnect dispatch is exposed.
- Distinct Shutdown card uses the existing Portable-only approval/token execution adapter. Runtime ownership generation, synchronous busy state, typed approval, refusal and uncertain execution results are preserved. Normal Safe Disconnect remains the existing one-press guarded whole-dock path. Unsupported combined sleep/shutdown entries remain unavailable.
- Settings Tutorials has four compact topics, native focus restore and source links. Wording approved by Wiki owner against candidate `04b1c1c`; actual Safe Disconnect/Diagnostics labels are used. Wiki publication is separate.
- Settings and Decky share About. Build-time virtual document module embeds exact LICENSE/THIRD_PARTY_NOTICES; no second attribution list. Controller buttons expand source text and open guide links. Historical legal names are exempted only as exact authoritative document literals in the branding test.
- Validation: 926 frontend tests; TypeScript; plugin build; 5 affected backend tests plus 7 subtests. The actual React fixture passes seven stages, including real native card activation through Safe Disconnect and Shutdown RPC seams, errors, double activation, unload during approval, and Tutorials/About controller focus. Decky/RPC ports are simulated; this is not native hardware validation. Prior validated hardware baseline remains f6059fa / 0.3.98.
- Source review/CI and later version allocation remain separate gates. No hardware mutation, restart or install occurred.
