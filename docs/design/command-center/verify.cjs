...
(async () => {
  ...
  const output = path.join(__dirname, 'captures'); fs.mkdirSync(output, { recursive: true });
  ...
  await page.locator('#stop-tdp').click();
  assert.match(await page.locator('.badge').innerText(), /Off/);
  await page.locator('#view-tdp').click();
  ...
  assert.equal(await page.locator('#stop-tdp').count(), 0);
  ...
  await page.screenshot({ path:path.join(output,'review-01.png'),fullPage:true });
  const result = { date: '2026-09-08', kind:'local HTML fixture; not native Decky', browser:await browser.version(), layoutCases:checks, interactionChecks:['Modules/page/Back route','focus return','Stop/Start local state','unknown recovery access','short-height focus scroll'], screenshots:22, runtimeErrors:errors, limitation:'Native QAM routing, gamepad, exact chrome, device capability and hardware actions not tested.' };
