...
for exact combinations and [Confirmed Hardware Testing](Confirmed-Hardware-Testing.md)
...
No. [Performance and Power](Performance-and-Power.md) depends on verified provider
...
Yes, bounded supervised sessions on the documented test profile have activated the TV and selected the external GPU. Automatic docking is experimental and off by default. Repeatable operation, recovery, and audio must still be verified for the exact build. See [Current State](Current-State.md).
