...
| --- | --- | --- | --- |
| Platform default | Unknown, degraded, disabled, missing telemetry, competing owner, or recovery | Restore/relinquish the Re-Gear-owned limit through the existing guarded path | Existing TDP owner only |
...
| Game starting | Trusted launch transition and exact identity; no game-performance telemetry assumed | Release any UI restriction promptly, before game demand | Existing TDP owner only |
...
| Game exiting | Confirmed exit, with a settling interval for shutdown/background work | Return to platform default before any UI-efficient request | Existing TDP owner only |

These are policy states, not a second independent writer state machine. `IDLE` from the game-session port means no identified game; it is insufficient by itself to apply a low power limit. `UNKNOWN` is not treated as idle. Paused, background, loading, shader-compilation, download, and docked variants remain observation labels until a reliable source and a reviewed response exist. Do not lower TDP from an unverified pause or quiet scene. A mode change during play leaves graphics and frame-generation configuration for the next launch; the current game must not be restarted or interrupted.
...
The Auto TDP side decides whether to consume this snapshot. It must compare identity, mode, context, ownership, launch generation, current power bounds, and **measured base-frame provenance** at dispatch time. A `base_fps_target` is a target, not a measurement. `target_display_fps` and Gamescope presented FPS must not be substituted for base FPS when frame generation is active. Profile lock or a successful configuration write cannot start a stopped Auto TDP session. User opt-out restores the prior platform policy through the existing guarded stop path.

## Control policy to validate before implementation

Power-up should respond faster than power-down, while both use explicit dwell times and fresh independent samples. A single slow frame, loading transition, or shader-compilation spike should not teach a new efficiency setting. Treat thermal pressure, power-source changes, an external writer, missing readback, and game/context change as epoch boundaries; reset evidence and recover to platform default. A CPU-bound classification or efficiency knee needs comparable measurements across scenes and real package/system power data. Configured watts are a limit, not consumed watts; do not infer total battery draw from APU TDP. Battery/AC may choose different reviewed objectives but AC is not automatic maximum power. Do not alter brightness, radios, refresh rate, or Gamescope limits in this slice.

No universal idle wattage, headroom value, base-FPS threshold, or polling frequency is justified yet. A future controller should take platform-observed legal bounds and reviewed policy parameters, reserve a UI responsiveness floor, and measure its own CPU, memory, wakeups, and battery cost. Restore after process failure and writer-lease loss must be demonstrated before any live low-power mode.
...
Supervised hardware validation is a later, separate gate: record the stock SteamOS menu and game baselines; measure UI responsiveness and total system draw; try one conservative UI limit with immediate restoration; then separately evaluate game Auto TDP in quiet and demanding scenes. Review latency, frame pacing, thermal response, suspend/crash restoration, and controller overhead before considering unattended operation. No hardware power limit or configuration is changed by this architecture work.
