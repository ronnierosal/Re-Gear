...
Re-Gear development source includes selected-game readiness guidance using local Steam evidence. See [Current State](../technical/current-state.md) and [source review](https://github.com/ronnierosal/Re-Gear/blob/main/docs/OFFLINE_EVIDENCE_SOURCE_REVIEW.md).
...
See [Confirmed Hardware Testing](../technical/hardware-evidence.md) for recorded device results. This Wiki does not claim that every game, or any particular untested game, will work offline.
...
The recent focused-refresh correction resumes checks after interrupted activity and bounds retries. If the view still reports unavailable or uncertain evidence, do not interpret that as a positive result. Report the game-view context, Re-Gear build, displayed reason, and what happened before the check stopped through [Troubleshooting](troubleshooting.md).
