...
- `stage_decky_update.py` and `ally_hdm_deploy_helper.py`: new root and matching
...
- `ally_hdm_deploy_helper.py` and `deploy_hdm_to_ally.ps1`: fixed new target and
...
`hdm` Python imports, `hdm-diagnose`, npm/Python distribution name
`handheld-dock-mode-steamos`, helper executable/script names, RPC and settings
keys, HDM diagnostic codes, and managed markers are retained. None determines
the archive root. Renaming them would enlarge this change into unrelated API,
state-schema or recovery-authority migration.

`/var/lib/handheld-dock-mode`, existing config paths and `.hdm-deploy-backups`
remain stable for recovery journals, preferences, deploy verification key and
existing rollback authority. Do not clear or rename them. Dated records and old
ZIPs retain their original paths, names, bytes and checksums.
