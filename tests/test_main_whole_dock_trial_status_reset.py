...
class WholeDockTrialStatusResetTests(unittest.TestCase):
    ...
    def test_retired_old_claim_and_stable_new_attachment_reset_terminal_view(self):
        ...
        self.assertEqual(self.plugin._whole_dock_trial_status, result)
        self.assertNotIn("request_id", result)

    def test_claim_inventory_and_identity_uncertainty_preserve_terminal_evidence(self):
        cases = (
            ("claim-present", (NS(stage="software_down"),), (TOKEN_A, TOKEN_A)),
            ("claim-appears", (None, NS(stage="claimed")), (TOKEN_A, TOKEN_A)),
            ("attachment-changes", (None, None, None), (TOKEN_A, TOKEN_B)),
        )
        for name, claims, tokens in cases:
    ...
    def test_ambiguous_or_failed_observation_preserves_terminal_evidence(self):
        store = Mock()
        store.load.return_value = None
        for name, scan in (
            ("none", []),
            ("many", [NS(boot_vga=False, pci_bdf="a"),
                      NS(boot_vga=False, pci_bdf="b")]),
            ...
            with self.subTest(name=name):
                ...
                if isinstance(scan, Exception):
                    discovery.scan.side_effect = scan
                else:
                    discovery.scan.return_value = scan
                with patch.object(self.module, "WholeDockClaimStore", return_value=store), \
                        patch.object(self.module, "DrmDiscovery", return_value=discovery):
                    result = self.read()
                self.assertEqual(result["code"], "dock_teardown.software_down")

    def test_busy_unresolved_and_malformed_success_are_never_reset(self):
        statuses = (
            {"schema_version": 1, "code": "dock_teardown.trial_running",
             "busy": True, "safe_to_unplug": False},
            {"schema_version": 1, "code": "dock_teardown.trial_unresolved",
             "busy": False, "ok": False, "safe_to_unplug": False},
            {"schema_version": 1, "code": "dock_teardown.software_down",
             "busy": False, "ok": False, "software_down": True,
             "safe_to_unplug": False},
        )
        for status in statuses:
            with self.subTest(code=status["code"], ok=status.get("ok")):
                ...
                self.assertNotEqual(result["code"], "dock_teardown.no_trial")

    def test_new_trial_started_during_observation_wins_compare_and_set(self):
        ...
        self.assertEqual(result["request_id"], "new-request")
