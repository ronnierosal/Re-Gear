...
class GoldenGateTests(unittest.TestCase):
    ...
    def coverage_shortfall(self, behaviors):
        """Everything the pinned floor requires that `behaviors` no longer has.

        A floor in both directions: a new behavior, or an extra test on an
        existing one, is never a shortfall. Only losing pinned coverage is.
        """
        shortfall = []
        for name, pinned in self.GOLDEN_COVERAGE.items():
            claimed = behaviors.get(name)
            if claimed is None:
                shortfall.append(f"behavior {name} is gone from the manifest")
                continue
            shortfall.extend(
                f"{name} no longer claims {test}"
                for test in pinned
                if test not in claimed
            )
        return shortfall
    ...
    def test_repository_manifest_preserves_every_pinned_behavior_and_test(self):
        ...
        self.assertEqual(
            self.coverage_shortfall(behaviors),
            [],
            "The manifest lost golden coverage it used to carry. Renaming or "
            "replacing a golden test must preserve its assertions and update "
            "this floor in the same change, with an explanation; dropping one "
            "silently is the regression this guard exists to block.",
        )

    def test_floor_admits_additions_and_still_catches_every_weakening(self):
        """The floor must never become a ceiling: new contracts have to pass."""
        pinned = {name: list(tests) for name, tests in self.GOLDEN_COVERAGE.items()}
        grown = {name: list(tests) for name, tests in pinned.items()}
        grown["automatic-connection-disconnect-coexistence"] = [
            "tests.test_whole_dock_runtime.WholeDockRuntimeTests.test_added_contract"
        ]
        grown["automatic-tv-readiness"].append(
            "tests.test_connection_readiness.ConnectionReadinessTests.test_added_later"
        )
        self.assertEqual(
            self.coverage_shortfall(grown),
            [],
            "An added behavior or an added test must not be reported as a "
            "shortfall; a guard that blocks new golden coverage discourages "
            "exactly the change it should encourage.",
        )

        victim = "restoration-before-presentation"
        donor = self.GOLDEN_COVERAGE["automatic-tv-readiness"][0]
        weakened = {
            "behavior deleted": {
                name: tests for name, tests in pinned.items() if name != victim
            },
            "test dropped": {**pinned, victim: pinned[victim][:-1]},
            "test swapped, count unchanged": {
                **pinned,
                victim: [donor, *pinned[victim][1:]],
            },
            "manifest gutted": {
                "automatic-tv-readiness": pinned["automatic-tv-readiness"][:1]
            },
        }
        for label, behaviors in weakened.items():
            with self.subTest(weakening=label):
                self.assertNotEqual(self.coverage_shortfall(behaviors), [])
