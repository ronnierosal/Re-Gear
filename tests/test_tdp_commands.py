...
from regear.ports.tdp import TdpDispatchRejected
...
class TdpCommandTests(unittest.TestCase):
    def test_rejected_or_throwing_dispatch_guard_never_starts_subprocess(self):
        ...
        for guard in (lambda: False, lambda: 1, fail):
            with self.subTest(guard=guard), patch("regear.adapters.steamos.commands.subprocess.run") as run:
    ...
    def test_guard_is_evaluated_after_identity_setup_and_is_not_retained(self):
        ...
        with patch("regear.adapters.steamos.commands.subprocess.run", side_effect=lambda *a, **k: order.append("dispatch") or self.completed()):
    ...
    def test_exact_read_argv_and_clean_bounded_subprocess(self):
        with patch("regear.adapters.steamos.commands.subprocess.run", return_value=self.completed()) as run:
    ...
    def test_exact_write_argv(self):
        with patch("regear.adapters.steamos.commands.subprocess.run", return_value=self.completed(stdout=b"")) as run:
    ...
    def test_exact_owner_argv(self):
        with patch("regear.adapters.steamos.commands.subprocess.run", return_value=self.completed(stdout=b's ":1.123"\n')) as run:
    ...
    def test_invalid_watt_values_never_invoke_subprocess(self):
        for watts in (True, False, 0, -1, 17.0, "17", None, float("nan"), float("inf"), 1 << 32):
            with self.subTest(watts=watts), patch("regear.adapters.steamos.commands.subprocess.run") as run:
    ...
    def test_invalid_or_mismatched_user_context_never_invokes_subprocess(self):
        ...
        for user in invalid:
            with self.subTest(user=user), patch("regear.adapters.steamos.commands.subprocess.run") as run:
    ...
    def test_other_effective_uid_refused(self):
        for uid in (-1, 1001, False, "0"):
            with self.subTest(uid=uid), patch("regear.adapters.steamos.commands.subprocess.run") as run:
    ...
    def test_timeout_and_unavailable_errors_are_categorical(self):
        for exception, expected in (
            ...
            with self.subTest(exception=exception), patch("regear.adapters.steamos.commands.subprocess.run", side_effect=exception):
                result = self.runner.read(self.user)
    ...
    def test_nonzero_redacts_both_streams(self):
        with patch("regear.adapters.steamos.commands.subprocess.run", return_value=self.completed(b"private output", b"private error", 1)):
            result = self.runner.read(self.user)
    ...
    def test_combined_output_limit_applies_to_reads_and_writes(self):
        for stdout, stderr in ((b"x" * 4097, b""), (b"x" * 2048, b"x" * 2049)):
            with self.subTest(lengths=(len(stdout), len(stderr))), patch("regear.adapters.steamos.commands.subprocess.run", return_value=self.completed(stdout, stderr)):
    ...
    def test_non_ascii_read_output_rejected(self):
        with patch("regear.adapters.steamos.commands.subprocess.run", return_value=self.completed(b"u \xff\n")):
