"""Arm and disarm the cgroup device filter behind `hdm.ports.device_filter`.

`hdm.delivery.device_filter_kernel` already supplies the syscall primitives and
`hdm.egpu_release` drives them by hand. Nothing implemented the port, so the
...
disappears with the process, which `hdm.application.filter_arm` is explicit
