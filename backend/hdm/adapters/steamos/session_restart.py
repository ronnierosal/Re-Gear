...
audio holders the target does not reach. `hdm.egpu_disconnect` prints the
commands and waits for an operator to run them, because `subprocess` is
confined to `hdm.adapters.steamos.commands` and nothing there could restart
...
already approved: `hdm.domain.filter_arm_sequence.APPROVED_EXPLICIT_RESTARTS`
