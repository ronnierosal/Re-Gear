...
`hdm.ports.filter_ownership` states the ordering this has to support: the record
...
as `hdm.delivery.removal_transaction_store`, which sits in the same directory
...
finished with is `hdm.domain.filter_ownership.reconcile`'s question, and this
...
def encode(record: FilterOwnership) -> bytes:
    ...
    `hdm.adapters.steamos.owner_identity` before it ever reaches a grant.
    """
