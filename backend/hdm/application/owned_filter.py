...
`hdm.application.filter_arm` is explicit that an unpinned link vanishing with
...
The ordering is the one `hdm.ports.filter_ownership` requires, and the reason for
...
is not an authorization -- `hdm.domain.filter_authorization` is -- and it is not
