    ...
    def build(
        ...
        payload: dict[str, Any] = {
            ...
            "versions": sanitize_value(
                {
                    # Older callers used the former product name. Normalize that
                    # input while publishing the current name consistently.
                    "regear": versions.get("regear", versions.get("hdm", "unknown")),
                    **{key: versions.get(key, "unknown") for key in ("decky", "steamos", "kernel")},
                },
                sensitive,
            ),
