...
class PresentationActivationApprovalStore:
    ...
    def issue(
        ...
        *,
        ...
        permit = PresentationActivationPermit(token, generation, user, fingerprint, prior_fingerprint)
        with self._lock:
...
class PresentationActivationService:
    ...
    def preview(self, *, user_confirmed: bool) -> PresentationActivationPreview:
        ...
        user, fingerprint, prior_fingerprint, status, blockers = self._preflight(observed.snapshot)
    ...
    def _execute_locked(self, token: str) -> PresentationActivationOutcome:
        ...
        user, fingerprint, prior_fingerprint, _, blockers = self._preflight(observed.snapshot)
        if (
            blockers
            or observed.generation != permit.generation
            or user != permit.user
            or fingerprint != permit.fingerprint
            or prior_fingerprint != permit.prior_fingerprint
        ):
            return PresentationActivationOutcome(False, False, "activation.evidence_changed")
        result = self._integration.activate()
        if not result.ok:
            return self._rollback(result.changed, "activation.install_failed", user)
        try:
        ...
        if (
            ...
            if result.changed:
                removed = self._integration.rollback_activation()
                return PresentationActivationOutcome(
                    False,
                    True,
                    (
    ...
    def _preflight(self, snapshot):
        ...
        if status.error_code and status.error_code != "managed_dropin_superseded":
        ...
        try:
            ...
            prior_fingerprint = self._integration.preparation_fingerprint()
        except (OSError, ValueError):
            fingerprint = ""
            prior_fingerprint = ""
            blockers.append("integration.fingerprint_unavailable")
        return user, fingerprint, prior_fingerprint, status, tuple(dict.fromkeys(blockers))
