...
The companion to ``regear.egpu_release``. That tool clears the holders of an eGPU
...
It never clears holders; run ``regear.egpu_release`` first if something still holds
...
Usage on the device, from the installed plugin directory:
    ...
    PYTHONPATH=backend python3 -m regear.egpu_remove
    sudo PYTHONPATH=backend python3 -m regear.egpu_remove --remove
    sudo PYTHONPATH=backend python3 -m regear.egpu_remove --rescan
"""
...
def egpu_functions(gpu_bdf: str) -> tuple[str, str]:
    ...
    Matches `regear.egpu_release.egpu_functions`; both describe the same device.
    """
...
NEXT_ACTION: dict[str, tuple[str, ...]] = {
    "removal_safety.clients_active_or_protected": (
        "holders are the blocker: clear them with regear.egpu_release --arm.",
    ),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="regear-egpu-remove", description=__doc__.split("\n\n")[0]
    )
