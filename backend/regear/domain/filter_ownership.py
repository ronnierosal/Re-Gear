...
`regear.adapters.steamos.device_filter` attaches is **unpinned**, so it disappears
...
`regear.delivery.device_filter_lifecycle` and `regear.delivery.device_filter_journal`,
...
behind `regear.ports.filter_ownership`.
"""
...
class AttachedFilter:
    ...
    The same three values `regear.ports.device_filter.ArmedFilter` carries, held
