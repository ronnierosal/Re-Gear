...
`regear.egpu_remove` handles that correctly while it is alive -- it stops at the
...
(`regear.domain.filter_ownership`) leaves the system unfiltered, which is the
...
behind `regear.ports.removal_transaction`. Nothing here is a claim that any device
is safe to unplug.
"""
