...
`regear.egpu_release` clears the holders and stops. `regear.egpu_remove` classifies
...
Usage on the device, from a source checkout:
    ...
    PYTHONPATH=backend python3 -m regear.egpu_disconnect
    sudo PYTHONPATH=backend python3 -m regear.egpu_disconnect --execute --release-display
"""
...
#: nowhere, which is the failure `regear.egpu_remove` fixed for its own codes.
NEXT_ACTION: dict[LiveDisconnectStage, tuple[str, ...]] = {
    LiveDisconnectStage.RELEASE_REFUSED: (
        "the release did not reach a clear device; run regear.egpu_release --arm",
    ...
    LiveDisconnectStage.RECOVERY_FAILED: (
        ...
        "device is half attached. Restore it with regear.egpu_remove --rescan.",
    ),
    LiveDisconnectStage.REMOVAL_UNRECOVERABLE: (
        "a function did not detach and the restore failed. The device is half",
        "attached. Restore it with regear.egpu_remove --rescan.",
    ),
...
def describe(result, *, gpu_bdf: str) -> None:
    ...
    if result.ok:
        ...
        report(f"  Restore them with: regear.egpu_remove --rescan --gpu {gpu_bdf}")
        return
...
def main(argv: Sequence[str] = ()) -> int:
    parser = argparse.ArgumentParser(
        prog="regear-egpu-disconnect", description=__doc__.split("\n\n")[0]
    )
