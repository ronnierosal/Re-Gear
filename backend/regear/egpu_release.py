...
Usage on the device, from the installed plugin directory:
    ...
    sudo PYTHONPATH=backend python3 -m regear.egpu_release
    sudo PYTHONPATH=backend python3 -m regear.egpu_release --arm --hold 120
    sudo PYTHONPATH=backend python3 -m regear.egpu_release --arm --hold 120 \
        --hold-open 180
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="regear-egpu-release", description=__doc__.split("\n\n")[0]
    )
    ...
    try:
        with CgroupDeviceLink() as link:
            ...
            if arguments.hold_open > 0:
                ...
                report(
                    "    sudo PYTHONPATH=backend python3 -m regear.egpu_remove"
