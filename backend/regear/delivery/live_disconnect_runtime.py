...
`regear.egpu_disconnect` composes the same pieces for an operator at a terminal.
...
def _sequence_would_clear(
    ...
    display. `regear.domain.disconnect_sequence` says why -- the verdict is being
...
def build_live_disconnect_runtime(
    *,
    ...
    The same pieces `regear.egpu_disconnect` composes for an operator, with one
