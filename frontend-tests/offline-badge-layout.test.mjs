...
test("matches visible Steam symbol size and center including scaled artwork", () => {
 for (const scale of [0.75, 1, 1.5]) {
  ...
  const result=offlineBadgeLayout(host,300,150,icon);
  assert.equal(result.height,32); assert.equal(result.width,64);
  assert.equal(150-result.bottom-result.height/2,130);
 }
});
test("narrow artwork keeps matching size without covering native icon", () => {
 ...
 assert.equal(result.height,32); assert.ok(150-result.bottom <= 116);
});
