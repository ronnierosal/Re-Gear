...
test("matches visible Steam symbol size and aligns beside its group including scaled artwork", () => {
 for (const scale of [0.75, 1, 1.5]) {
  ...
  assert.equal(result.height,20); assert.equal(result.width,20);
  assert.equal(150-result.bottom,140);
...
test("narrow artwork keeps matching size without covering native icon", () => {
 ...
 assert.equal(result.height,20); assert.equal(150-result.bottom,140);
 assert.equal(result.left,6);
});
