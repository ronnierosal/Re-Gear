...
try{for(const before of [true,false]){
 ...
 if(geometry.disconnect.bottom>geometry.content.bottom)throw Error('Safe Disconnect hidden');
 if(!before){
 ...
 const railOverflow=await page.locator('.rg-utility-rail').evaluateAll(rails=>rails.some(r=>r.scrollHeight>r.clientHeight+1));if(railOverflow)throw Error('Utility controls not visible immediately');
 const right=await page.locator('[data-utility-side="right"]').boundingBox();
 if(Math.abs(viewport.width-right.x-right.width-viewport.width*.02)>2)throw Error('Right controls not at screen edge');
 ...
 if(prior.geometry.panel.width!==geometry.panel.width||prior.geometry.panel.height!==geometry.panel.height)throw Error('Central dimensions changed');
 }
 ...
 await writeFile('out/utility-preview/report.json',JSON.stringify({baseline,head:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),simulated:true,reports},null,2));console.log('Four same-size source comparisons passed; navigation, disabled controls, central dimensions and immediate visibility checked.');
