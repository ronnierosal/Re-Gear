...
async function checkDisconnectRecovery(page){
 // Reset only simulated persistent history between these independent cases.
 await page.evaluate(()=>{window.localStorage.removeItem('regear.whole-dock.pending-request');window.egpuUp=true;window.rejectTrialOnce=true;});
 await page.waitForFunction(()=>window.retainedView.disconnectControl.props.readCurrentSnapshot()?.egpu_link?.state==='up');
 const count=()=>page.evaluate(()=>window.rpcCalls.filter(x=>x.name==='execute_egpu_disconnect').length);
 const before=await count();await page.locator('[data-ec-tab=egpu]').click();await page.locator('[data-ec-control=disconnect]').click();
 const recovery=page.locator('#operation').getByRole('button',{name:'Safe Disconnect',exact:true});await recovery.waitFor();
 assert.match(await page.locator('#operation [role=status]').textContent(),/No request was sent by this attempt/);
 assert.equal(await recovery.getAttribute('type'),'button');
 await page.waitForFunction(()=>document.querySelector('#operation button')?.disabled===false);
 assert.equal(await count(),before,'later fresh poll never submits');
 const message=await page.locator('#operation [role=status]').textContent();assert.ok(!message.includes('unavailable'),'recovery message follows current ready observation');
 const modalCount=await page.evaluate(()=>window.modalShows);
 await recovery.evaluate(n=>{n.dispatchEvent(new CustomEvent('gamepadok',{bubbles:true,cancelable:true}));n.dispatchEvent(new CustomEvent('gamepadok',{bubbles:true,cancelable:true}));});
 await page.waitForFunction(before=>window.rpcCalls.filter(x=>x.name==='execute_egpu_disconnect').length===before+1,before);
 assert.equal(await page.evaluate(()=>window.modalShows),modalCount,'explicit recovery uses direct route, no new confirmation');
 assert.equal(await page.evaluate(()=>window.modalSubmissions??0),0,'inner recovery does not submit the parent form');
 assert.equal(await recovery.count(),0,'submitted request never exposes retry');
 await page.evaluate(()=>{window.lastModal.props.onCancel();window.egpuUp=false;});
 console.log('PASS native recovery popup after first-read failure: fresh polling updates reason without dispatch, explicit A executes once, type=button preserves parent dialog, no submitted retry');
}
